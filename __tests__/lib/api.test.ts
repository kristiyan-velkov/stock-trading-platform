import { fetchStockData, fetchHistoricalData } from "@/lib/server/api";
  ...
  describe("fetchStockData", () => {
    it("should fetch stock data for given symbols", async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        expect.stringContaining("price?symbol=AAPL")
      );
      expect(global.fetch).toHaveBeenCalledWith(
        expect.stringContaining("quote?symbol=AAPL")
      );
      expect(global.fetch).toHaveBeenCalledWith(
        expect.stringContaining("time_series?symbol=AAPL")
      );
    });
    ...
    it("should fetch historical data for a given symbol and interval", async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        expect.stringContaining(
          `time_series?symbol=${mockSymbol}&interval=${mockInterval}`
        )
      );
    });
